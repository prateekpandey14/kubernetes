...
import (
	...
	svc "k8s.io/kubernetes/pkg/registry/core/service"
...
func NewGenericREST(optsGetter generic.RESTOptionsGetter, serviceCIDR net.IPNet, hasSecondary bool) (*GenericREST, *StatusREST, error) {
	strategy, _ := svc.StrategyForServiceCIDRs(serviceCIDR, hasSecondary)
	...
	statusStore := *store
	statusStrategy := svc.NewServiceStatusStrategy(strategy)
...
func (r *GenericREST) defaultOnReadService(service *api.Service) {
	...
	svc.NormalizeClusterIPs(nil, service)
